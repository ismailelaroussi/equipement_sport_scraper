...
import random
import time
import webbrowser  # Ajouté pour pouvoir ouvrir les liens dans le navigateur
...
class RobotSport:
        ...
        try:
            ...
            if site == "Decathlon MA":
                ...
                for fiche in fiches[:15]:
                    ...
                    # Extraction du lien
                    lien_tag = fiche.find('a')
                    lien_art = lien_tag['href'] if lien_tag and lien_tag.has_attr('href') else lien
                ...
                for fiche in fiches[:12]:
                    ...
                    # Extraction du lien générique
                    lien_tag = fiche.find('a') if hasattr(fiche, 'find') else None
                    if lien_tag and lien_tag.has_attr('href'):
                        lien_art = lien_tag['href']
                        if lien_art.startswith('/'):
                            # Reconstruire le lien absolu si c'est un lien relatif
                            base_url = "/".join(lien.split("/")[:3])
                            lien_art = base_url + lien_art
                    else:
                        lien_art = lien
    ...
    def secours(self, site, recherche=""):
        ...
        lien_de_secours = urls.get(site, "https://google.com")
        ...
        for _ in range(15):
            ...
            items.append({
                ...
                "lien": lien_de_secours  # Ajout du lien de secours
            })
...
class AppSport:
    def __init__(self, fenetre):
        ...
        self.fenetre.geometry("1100x700")  # Élargi un peu pour la colonne lien
    ...
    def interface(self):
        ...
        # Ajout de "lien" dans le tuple des colonnes
        colonnes = ("site", "nom", "prix", "taille", "couleur", "lien")
        ...
        self.tableau.heading("lien", text="Lien de l'article")  # En-tête du lien

        self.tableau.column("site", width=100, anchor="center")
        self.tableau.column("nom", width=300, anchor="w")
        self.tableau.column("prix", width=90, anchor="center")
        self.tableau.column("taille", width=70, anchor="center")
        self.tableau.column("couleur", width=100, anchor="center")
        self.tableau.column("lien", width=250, anchor="w")  # Largeur colonne lien

        # Liaison du double-clic pour ouvrir le lien
        self.tableau.bind("<Double-1>", self.ouvrir_lien)
        ...
        self.lbl_total = ttk.Label(f_bas, text="Articles : 0 (Double-cliquez sur une ligne pour ouvrir le lien)", font=("Helvetica", 10, "italic"), background="#f8f9fa")
    ...
    def afficher(self, liste):
        ...
        for i in liste:
            self.tableau.insert("", "end", values=(
                ...
                i["taille"],
                i["couleur"],
                i["lien"]  # Insertion du lien dans le tableau
            ))
        self.lbl_total.config(text=f"Articles : {len(liste)} (Double-cliquez sur une ligne pour ouvrir le lien)")
    ...
    def ouvrir_lien(self, event):
        # Récupère l'élément sélectionné lors du double-clic
        item_selectionne = self.tableau.focus()
        if item_selectionne:
            valeurs = self.tableau.item(item_selectionne)["values"]
            # Le lien se trouve à l'index 5 (6ème colonne)
            url_article = valeurs[5]
            if url_article:
                webbrowser.open(url_article)
        ...
        if destination:
            try:
                with open(destination, mode="w", newline="", encoding="utf-8-sig") as f:
                    ...
                    writer.writerow(["Site", "Article", "Prix", "Taille", "Couleur", "Lien"])
                    for ligne_id in lignes:
                        writer.writerow(self.tableau.item(ligne_id)["values"])
                messagebox.showinfo("Succès", "Fichier sauvegardé avec les liens.")
            except Exception as e:
